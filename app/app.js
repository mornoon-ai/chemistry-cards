(function () {
  ...
  const stateKey = "chemistry-card-state-v1";
  const els = {
  ...
  function enhanceChem(value) {
    function subscriptFormula(formula) {
      return formula.replace(/([A-Z][a-z]?|\))(\d+)/g, "$1<sub>$2</sub>");
    }
    return value
      .replace(/\b((?:[A-Z][a-z]?\d*){2,})([+-])(?=$|[^A-Za-z0-9])/g, (_, formula, charge) => {
        return `${subscriptFormula(formula)}<sup>${charge}</sup>`;
      })
      .replace(/\b([A-Z][a-z]?)(\d+)([+-])(?=$|[^A-Za-z0-9])/g, "$1<sup>$2$3</sup>")
      .replace(/\b([A-Z][a-z]?)([+-])(?=$|[^A-Za-z0-9])/g, "$1<sup>$2</sup>")
      .replace(/([A-Z][a-z]?|\))(\d+)/g, "$1<sub>$2</sub>")
      .replace(/e-/g, "e<sup>-</sup>")
      .replace(/e⁻/g, "e<sup>-</sup>");
  }
  ...
  function renderContent(text) {
    ...
    if (lines.every((line) => line.trim().startsWith("- ") || line.trim() === "")) {
      const items = lines
        ...
        .map((line) => `<li>${enhanceChem(line.trim().replace(/^- /, ""))}</li>`)
        .join("");
      return `<ul>${items}</ul>`;
    }
    const markdown = safe
